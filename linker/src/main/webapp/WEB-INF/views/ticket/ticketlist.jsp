...
<input type="date" id="start-date-input" name="start-date" min="2021-01-01" max="" value="" required oninput="restrictEndDate()">

<label for="end-date-input"><a id="P">~</a></label>
<input type="date" id="end-date-input" name="end-date" min="" max="" value="" required oninput="restrictStartDate()">
...
function setMaxDate(elementId){
	...
	var currentDateNum = currentDate.getDate() < 10 ? "0" + currentDate.getDate() : currentDate.getDate();
	var currentDateString = currentYear + "-" + currentMonth + "-" + currentDateNum;
	
	document.getElementById(elementId).setAttribute("max", currentDateString);
}

function restrictEndDate(){
	var startDateValue = document.getElementById("start-date-input").value;
	document.getElementById("end-date-input").setAttribute("min", startDateValue);
}

function restrictStartDate(){
	var endDateValue = document.getElementById("end-date-input").value;
	document.getElementById("start-date-input").setAttribute("max", endDateValue);
}
...
function resetSearch(){
	...
	location.assign("${pageContext.request.contextPath}/ticket/ticketlist");
...
function showSaledResult(start, end, page) {
	   ...
	   //[{"totalPages":6,"count":55,"end":5,"pageNum":5,"begin":1},
	    ...
	    var page_str = ""; //사용될 HTML 문자열을 저장할 page_str 변수를 선언하고 빈 문자열을 할당함
	   	...
	   	if(pageData.begin > pageData.pageNum ){
			...
			page_str += "<span onclick=showSaledResult('"+ pageData.startDate+ "','" + pageData.endDate+ "','" + (pageData.begin - 1) +"')> 이전 </span> ";
		}
	   	
	    for(index = pageData.begin ; index <= pageData.end; index++){
	      	
	    	page_str += "<span onclick=showSaledResult('"+ pageData.startDate+ "','" + pageData.endDate+ "','" + index+"')>" +index+' </span> ';
	    	
	    }
	    
		if(pageData.end < pageData.totalPages ){
			
			page_str += "<span onclick=showSaledResult('"+ pageData.startDate+ "','" + pageData.endDate+ "','" + (pageData.end +1) +"')> 다음 </span> ";
		}
 ...
 <div id="page">
 	<c:if test="${begin > pageNum }">
 		<a href="ticketlist?p=${begin-1 }">[이전]</a>
 	</c:if>
 	<c:forEach begin="${begin }" end="${end }" var="i">
 		<a href="ticketlist?p=${i }">${i }</a>
 	</c:forEach>
 	<c:if test="${end < totalPages }">
 		<a href="ticketlist?p=${end+1 }">[다음]</a>
 	</c:if>
 </div>
 	<c:if test="${count == 0 }">
 	아직 구매한 식권이 없습니다.
